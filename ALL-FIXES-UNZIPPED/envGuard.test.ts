...
describe('validateEnv', () => {
    ...
    it('passes when both OAuth parts are set', () => {
        const result = validateEnv(createMockEnv({
            GOOGLE_CLIENT_ID: 'some-id',
    ...
    it('passes when both OAuth parts are empty', () => {
        const result = validateEnv(createMockEnv({
            GITHUB_CLIENT_ID: '',
            GITHUB_CLIENT_SECRET: '',
        }));
        expect(result.warnings.filter(w => w.includes('GitHub OAuth'))).toHaveLength(0);
    });
});
