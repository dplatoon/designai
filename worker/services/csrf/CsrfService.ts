...
export class CsrfService {
    ...
    static getTokenFromCookie(request: Request): string | null {
        ...
        try {
            ...
            // Use a safe default TTL (2 hours) when env is not available in this context
            const defaultTTL = 2 * 60 * 60 * 1000;
            if (tokenAge > defaultTTL) {
    ...
    static async enforce(
        ...
        if (request.method === 'GET' && response) {
            ...
            if (!existingToken) {
                ...
                const tokenTTL = env ? this.getDefaults(env).tokenTTL : 2 * 60 * 60 * 1000;
